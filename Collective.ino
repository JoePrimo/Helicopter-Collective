...
#define joyButtonRotory 14 //adding additional buttons not included in button matrix
#define joyButtonMouse 15
#define joyButtonRoller 16
...
byte rowPins[NUMROWS] = {A7,A6,A8,A9}; //change "?" to the pins the rows of your button matrix are connected to
byte colPins[NUMCOLS] = {2,3,5,7}; //change "?" to the pins the rows of your button matrix are connected to
...
void setup() {
  pinMode(joyButtonRotory, INPUT_PULLUP);
  pinMode(joyButtonMouse, INPUT_PULLUP);
  pinMode(joyButtonRoller, INPUT_PULLUP);
...
void CheckAllPotentiometers(){
  ...
  //Non matrix buttons setup
  int currentButton1State = !digitalRead(joyButtonRotory);
  if (currentButton1State != lastButton1State){
  Joystick.setButton (18, currentButton1State);
  lastButton1State = currentButton1State;
  }

    int currentButton2State = !digitalRead(joyButtonMouse);
  if (currentButton2State != lastButton2State){
  Joystick.setButton (19, currentButton2State);
  lastButton2State = currentButton2State;
  }

  int currentButton3State = !digitalRead(joyButtonRoller);
  if (currentButton3State != lastButton3State){
  Joystick.setButton (20, currentButton3State);
